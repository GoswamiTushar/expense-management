...
export default function DashboardContent({ data, cat, setCat, setM, setSettleWith }) {
  ...
  return (
    <ScrollView refreshControl={<RefreshControl refreshing={data.refreshing} onRefresh={data.refresh} tintColor={colors.primary} />}>
      {!data.activeProperty ? renderEmpty() : (
        <>
          ...
          <View style={styles.sectionHeader}>
            <View style={styles.sectionTitleBox}><Text style={styles.title}>Logged Expenses</Text><Text style={styles.sub}>{data.expenses.length} expenses • Splitwise split</Text></View>
            <View style={styles.headerActions}>
              <CsvExportButton expenses={data.expenses} property={data.activeProperty} settlements={data.settlements} userMap={userMap} />
              <TouchableOpacity style={styles.btn} onPress={() => setM('showHistory', true)}><Ionicons name="receipt-outline" size={12} color={colors.textDark} /><Text style={styles.btnText}>Audit Log</Text></TouchableOpacity>
            </View>
          </View>
